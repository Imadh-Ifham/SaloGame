  ...
  try {
    ...
    if (transactionType === "walk-in-booking") {
      if (paymentType && paymentType !== "cash" && paymentType !== "card") {
        res.status(400).json({
          error:
            "Payment type for walk-in-booking must be cash or card if specified",
        });
        return;
      }
      ...
      if (req.user.role !== "owner" && req.user.role !== "manager") {
        res.status(403).json({
          error:
            "Only owners and managers can create walk-in-booking transactions",
        });
        return;
      }
    }
...
const updateLast30DaysTransactions = async (transaction: any) => {
  try {
    ...
    if (!last30DaysDoc) {
      ...
      last30DaysDoc.transactions = last30DaysDoc.transactions.filter(
...
export const updateTransaction = async (
  ...
  try {
    ...
    if (
      ...
      transaction.transactionType !== "walk-in-booking"
    ) {
      res.status(400).json({
